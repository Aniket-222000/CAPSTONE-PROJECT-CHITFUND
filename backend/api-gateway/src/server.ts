...
app.use(cors({
    origin:['http://localhost:4200', 'http://localhost:3007'],
...
app.use(
    ...
    createProxyMiddleware({
      target: process.env.AUDIT_SERVICE_URL || 'http://localhost:3005/api/audit',
      changeOrigin: true
    })
  );
  ...
  app.use(
    '/api/notify',
    createProxyMiddleware({
      target: process.env.NOTIFY_SERVICE_URL || 'http://localhost:3006/api/notify',
      changeOrigin: true
    })
  );
