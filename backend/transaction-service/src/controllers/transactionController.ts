...
export const getTransactionById = async (req: Request, res: Response): Promise<void> => {
  try {
    const txn = await service.getTransactionById(req.params.transactionId);
    if (!txn)  res.status(404).json({ message: 'Transaction not found' });
...
export const getTransactionsByUserId = async (req: Request, res: Response): Promise<void> => {
  try {
    const txns = await service.getTransactionsByUserId(req.params.userId);
    res.status(200).json(txns);
  } catch {
    res.status(500).json({ message: 'Failed to retrieve transactions' });
  }
...
export const getTransactionsByGroupId = async (req: Request, res: Response): Promise<void> => {
  try {
    const txns = await service.getTransactionsByGroupId(req.params.groupId);
    res.status(200).json(txns);
  } catch {
    res.status(500).json({ message: 'Failed to retrieve transactions' });
  }
...
export const getTransactionsByType = async (req: Request, res: Response): Promise<void> => {
  try {
    const txns = await service.getTransactionsByType(req.params.transactionType);
    res.status(200).json(txns);
  } catch {
    res.status(500).json({ message: 'Failed to retrieve transactions' });
  }
