...
const AdminRegistrationPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isValid = Object.keys(formData).every((key) =>
      key === 'userRole' ? true : validateField(key, formData[key as keyof typeof formData])
    );
    ...
    try {
      // Use admin-specific registration endpoint or add a secret key
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-blue-800 to-indigo-900">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-2xl">
        ...
        <form onSubmit={handleSubmit}>
              ...
              field !== 'userRole' && (
                <div key={field} className="mb-4">
                  ...
                  <input
                    ...
                    required
                    className={`w-full p-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors[field as keyof typeof errors] ? 'border-red-500' : ''}`}
                  />
          ...
          <button
            type="submit"
            className="w-full bg-blue-700 text-white p-3 rounded-md hover:bg-blue-600 transition duration-200 mt-6"
          >
            Register Admin
          </button>
        </form>
